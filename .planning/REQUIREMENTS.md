...
- [ ] **ENGINE-01**: Every move (play cards, pick up pile, swap cards, ready up) is processed through a single, pure `applyMove(state, move)` reducer built on `gameLogic.ts`'s existing predicates
- [ ] **ENGINE-02**: No move handler mutates existing state or player objects in place — every move produces a new state object
...
- [ ] **ENGINE-05**: Invalid moves and game feedback appear as in-app toast/inline messages instead of blocking `window.alert()` calls
- [ ] **ENGINE-06**: The player finish celebration animation (already written in `App.css`) actually plays in the browser
- [ ] **ENGINE-07**: The new engine and split screen components have automated test coverage (unit tests for the engine, smoke tests for the screens)
...
- [ ] **MPLAY-06**: Players can see when an opponent has disconnected mid-game (presence)
