...
- **D-05:** If a player doesn't act within ~60 seconds of their turn starting (grace period) **and** they are also stale by `DISCONNECT_THRESHOLD_MS` on the server-verified heartbeat signal, the server automatically executes `PICK_UP_PILE` on their behalf. This is always a legal move (no guessing what they'd have played), so it's safe to automate. They keep their hand/seat and resume playing normally whenever they reconnect. Chosen over "pause and wait" (stalls the whole table indefinitely) and "eliminate from round" (too harsh for a brief drop, and reshapes deck/turn-order math unnecessarily). The engine currently has **no "pass" move** - `PICK_UP_PILE` is the only always-legal action when a turn can't otherwise be resolved, which is why this was the natural mechanism rather than inventing a new one.
  **Revised 2026-07-28** after a live two-device play test surfaced the original turn-duration-only version auto-picking-up a fully connected player whose own turn simply ran long (confirmed against `player_seen`: the punished player's heartbeat was the *freshest* in the room). The grace-period timer alone cannot distinguish "present but slow" from "genuinely gone" - only the connectivity signal can, so the two checks are now both required. A present-but-slow player is never auto-picked-up; only a player who is both over the turn-duration grace period *and* stale on `player_seen` is.
